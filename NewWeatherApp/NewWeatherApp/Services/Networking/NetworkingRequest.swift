...
public class NetworkingRequest: NSObject {
    ...
    public func uploadPublisher() -> AnyPublisher<(Data?, Progress), Error> {
        guard let urlRequest = buildURLRequest() else {
            return Fail(error: NetworkingError.unableToParseRequest as Error).eraseToAnyPublisher()
        }
    ...
    fileprivate func handleParameterEncoding(_ request: inout URLRequest) {
        if httpMethod != .get && multipartData == nil {
            switch parameterEncoding {
            case .urlEncoded:
                request.setValue("application/x-www-form-urlencoded", forHTTPHeaderField: contentTypeKey)
            case .json:
    ...
    fileprivate func handleMultipartRequest(_ request: inout URLRequest) {
        // Multipart
    ...
    internal func buildURLRequest() -> URLRequest? {
        ...
        handleParameterEncoding(&request)
